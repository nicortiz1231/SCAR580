...
TSharedPtr<FJsonObject> FBPConnector::ConnectNodes(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Use the K2 schema to validate and create connections — handles wildcards,
    // implicit casts, and all Blueprint-native pin compatibility rules.
    const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
    FPinConnectionResponse ConnResponse = K2Schema->CanCreateConnection(SourcePin, TargetPin);
    if (ConnResponse.Response == CONNECT_RESPONSE_DISALLOW)
    {
        Result->SetBoolField("success", false);
        Result->SetStringField("error", FString::Printf(TEXT("Pins not compatible: %s"), *ConnResponse.Message.ToString()));
        return Result;
    }

    // Create connection (handles wildcard promotion, type coercion, etc.)

...
void showAssetDepotConfirmWindow(
  ...
  // Indirection so the Close lambda can capture a shared ownership handle
  // without referencing AssetDepotConfirmWindow during its own construction
  // (Clang -Werror/-Wuninitialized rejects that pattern).
  TSharedRef<TSharedPtr<SWindow>> WindowHolder =
      MakeShared<TSharedPtr<SWindow>>();
  // clang-format off
  *WindowHolder =
    SNew(SWindow)
        ...
        SNew(SVerticalBox) +
          ...
          [
            SNew(SButton)
              .OnClicked_Lambda(
                  [WindowHolder]() {
                    if (WindowHolder->IsValid())
                    {
                      (*WindowHolder)->RequestDestroyWindow();
                    }
  ...
  // clang-format on
  GEditor->EditorAddModalWindow(WindowHolder->ToSharedRef());
}
} // namespace
